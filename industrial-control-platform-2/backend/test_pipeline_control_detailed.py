...
def test_pipeline_control_detailed():
...
def test_device_connections():
    ...
    try:
        ...
        if response.status_code == 200:
            result = response.json()
            print(f"设备连接状态: {result}")
        else:
            print(f"设备连接状态获取失败: {response.status_code}")
            print(f"错误响应: {response.text}")
    except Exception as e:
        print(f"设备连接状态测试异常: {e}")

def test_conveyor_status():
    print("\n=== 测试传送带状态 ===")

    try:
        response = requests.get("http://127.0.0.1:5000/api/conveyor/status")
        if response.status_code == 200:
            result = response.json()
            print(f"传送带状态: {result}")
        else:
            print(f"传送带状态获取失败: {response.status_code}")
            print(f"错误响应: {response.text}")
    except Exception as e:
        print(f"传送带状态测试异常: {e}")
...
if __name__ == "__main__":
    ...
    test_conveyor_status()
    test_pipeline_control_detailed()